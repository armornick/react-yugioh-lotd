...
const OUTPUT_FNAME = 'gx3-boosters.json';
...
const RE_GX3_LINK = /_\(GX03-BP\)$/;

const linkMatcher = (link) => {
    return RE_GX3_LINK.test(link);
}

const parseLinks = (links) => {
    const results = [];
    for (const link of links) {
        const name = link.replace(RE_GX3_LINK,'').replace(/^\/wiki\//, '');
        results.push({
            name: name.replace(/_/g, ' ').replace('%27', '\''),
            fname: 'GX3_' + name + '.html',
...
const main = async () => {
    ...
    const response = await dl.download('GX3-Boosters.html', 'https://yugipedia.com/wiki/Template:Yu-Gi-Oh!_GX_Spirit_Caller_sets');
